...
const isResponsivePropType  = <T>(
    prop: unknown,
): prop is ResponsiveProp<T> => {
    if(typeof prop !== 'object' || prop === null) return false;
    
    const hasResponsivePropKey = (
        'base'  in prop ||
        'sm'    in prop ||
        'md'    in prop ||   
        'lg'    in prop ||
    ...
    return hasResponsivePropKey
}
const isSpaceThemeKeys = (prop: unknown, theme:AppTheme): prop is SpaceThemeKeys => {
    return Object.keys(theme.space).filter((key) => key === prop).length > 0;
}
const isColorThemeKeys = (prop: unknown, theme:AppTheme): prop is ColorThemeKeys => {
    return Object.keys(theme.colors).filter((key) => key === prop).length > 0;
}
const isFontSizeThemeKeys = (prop: unknown, theme:AppTheme): prop is FontSizeThemeKeys => {
    return Object.keys(theme.fontSizes).filter((key) => key === prop).length > 0;
}
const isLetterSpacingThemeKeys = (prop: unknown, theme:AppTheme): prop is LetterSpacingThemeKeys => {
    return Object.keys(theme.letterSpacings).filter((key) => key === prop).length > 0;
}
const isLineHeightThemeKeys = (prop: unknown, theme:AppTheme): prop is LineHeightThemeKeys => {
